...
//            if(PlaceBuf()) {
        ...
        NMinPCOut=0;
#endif
        if (!NumBlock && (GD.Hot.News&bInClock)) {

            datetime_t dt;
            control_time_to_datetime(&dt, &GD.Hot.time, 0);
            uint32_t new_ts = rtc_ts_from_dt(&dt);
            uint32_t our_ts = rtc_get();

            int64_t delta = (int64_t)our_ts - new_ts;
