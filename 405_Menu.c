...
//char nBlk;
    ...
    if (EndInput && (BlockEEP[nBlEEP].Erase ==1)) {
        ...
        if (nBlEEP == 5) {
            nBlEEP=7;
            BlockEEP[nBlEEP].Erase = 1;
            TestFRAM(nBlEEP+10);
        }

        nBlEEP=2;
        BlockEEP[nBlEEP].Erase = 1;
        TestFRAM(nBlEEP+10);        // в любом случае переписываем Точные настройки
//			RamToEEP(nBlEEP);
    }
}
...
//	w_int(&GD.FanBlock[0][0].FanData[0].ActualSpeed,SSSS);
    ...
    if (!ByteZ) {
        ...
        w_int(&GD.Hot.Tepl[ByteY].InTeplSens[cSmTSens5].Value,SSpS0);
        w_int(&GD.Hot.Tepl[ByteY].InTeplSens[cSmTSens6].Value,SSpS0);
        return;
    }
    if (ByteZ==1) {
        w_txt(Mes43);
...
//    w_int(&GD.Config[cfSumIrrigVal],SS);
    ...
    if (EndInput) {
        EndInput=0;
        if (GD.Hot.time.Time>=24*60)
        	GD.Hot.time.Time=1;


//		CalcDayOfWeek();
        datetime_t dt;
        control_time_to_datetime(&dt, &GD.Hot.time, 0);
        uint32_t ts = rtc_ts_from_dt(&dt);
        rtc_set(ts);
    }
}
